...
mod ax_rt {
    ...
    pub async fn start_server(port: u16, state: Arc<ServerState>) -> Result<(), Box<dyn std::error::Error>> {
        ...
        match state.router.match_route(&method, &path) {
            Ok(matched) => {
                ...
                if !matched.params.is_empty() {
                    let mut params_map = std::collections::HashMap::with_capacity(matched.params.len());
                ...
                if let Some(dto_name) = &matched.value.dto_class {
                    if let Some(v_reg) = &state.validator {
                        match v_reg.validate(dto_name, body_str) {
                            ...
                            Err(errors) => {
                                let err_json = serde_json::to_string(&errors).unwrap_or_else(|_| "{}".to_string());
                ...
                if res_len < 0 {
                    return (StatusCode::INTERNAL_SERVER_ERROR, "PHP Handler Error").into_response();
                }
                ...
                if let Some(headers) = php_res["headers"].as_object() {
                    ...
                    for (k, v) in headers {
                        if let (Ok(h_name), Ok(h_val)) = (HeaderName::from_bytes(k.as_bytes()), HeaderValue::from_str(v.as_str().unwrap_or(""))) {
...
#[no_mangle]
pub extern "C" fn quill_router_build(manifest_json: *const c_char, manifest_len: usize) -> *mut std::ffi::c_void {
...
pub extern "C" fn quill_router_match(
    ...
    method: *const c_char, method_len: usize,
    path: *const c_char, path_len: usize,
    out_handler_id: *mut u32,
    out_num_params: *mut u32,
    out_params_json: *mut c_char, out_params_max: usize,
) -> std::ffi::c_int {
    if router_ptr.is_null() || method.is_null() || path.is_null() || out_handler_id.is_null() || out_num_params.is_null() || out_params_json.is_null() {
...
pub extern "C" fn quill_json_compact(
    ...
    out_buf: *mut c_char, out_max: usize
) -> usize {
...
pub extern "C" fn quill_validator_register(
    ...
    schema_json: *const c_char, schema_len: usize,
) -> std::ffi::c_int {
...
pub extern "C" fn quill_validator_validate(
    registry_ptr: *mut std::ffi::c_void,
    dto_name: *const c_char, dto_name_len: usize,
    input_json: *const c_char, input_len: usize,
    out_json: *mut c_char, out_max: usize,
) -> std::ffi::c_int {
    ...
    let result = catch_unwind(|| {
        ...
        match registry.validate(name_str, input_str) {
            Ok(validated_val) => {
                let json_string = sonic_rs::to_string(&validated_val).unwrap_or_else(|_| "{}".to_string());
                let bytes = json_string.as_bytes();
            ...
            Err(errors) => {
                let json_string = serde_json::to_string(&errors).unwrap_or_else(|_| "{}".to_string());
                let bytes = json_string.as_bytes();
                let len = bytes.len().min(out_max - 1);
                unsafe {
                    std::ptr::copy_nonoverlapping(bytes.as_ptr(), out_json as *mut u8, len);
...
pub extern "C" fn quill_router_dispatch(
    ...
    method: *const c_char, method_len: usize,
    path: *const c_char, path_len: usize,
    body_json: *const c_char, body_len: usize,
    out_json: *mut c_char, out_max: usize,
) -> std::ffi::c_int {
                ...
                if let Some(dto_name) = &matched.value.dto_class {
                    if let Some(v_reg) = validator {
                        if !body_json.is_null() && body_len > 0 {
                            let body_slice = unsafe { slice::from_raw_parts(body_json as *const u8, body_len) };
                            ...
                            response.insert("dto_valid".to_string(), serde_json::json!(false));
                            response.insert("dto_errors".to_string(), serde_json::json!({"body": ["Request body is required"]}));
                        }
                    }
                }
            }
