...
impl QuillRouter {
    pub fn new(manifest_json: &str) -> Option<Self> {
        ...
        for entry in entries {
            let router = routers.entry(entry.method.clone()).or_insert_with(Router::new);
    ...
    pub fn match_route<'a>(&'a self, method: &str, path: &'a str) -> Result<Match<'a, 'a, &'a RouteMetadata>, i32> {
